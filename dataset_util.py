...
class DatasetUtil:
        ...
        if dataset == Dataset.GEO_LIFE:
            ...
            def extract_geo_life_trajectory(file_name, out_file_name, out_file_path):
                ...
                with open(file_name, "r") as f:
                    trajectory_data = list(map(lambda line: (float(line[0]), float(line[1]), 24*3600*float(line[4])), map(lambda line: line.split(","), f.readlines()[6:])))

                return trajectory_data
            Path(out_path).mkdir(parents=True, exist_ok=True)
            trajectories = Parallel(n_jobs=16, verbose = 1)(delayed(extract_geo_life_trajectory)(file, file.name, Configuration.EXTRACTED_DATA_PATH.format(dataset.value)) for file in file_names)
            with open(Path(out_path, "extracted_trajectories.dat"), "w") as f:
                json.dump(trajectories, f)

        elif dataset == Dataset.TAXI:
            file_name = Path(Configuration.RAW_DATA_PATH.format(dataset.value), "data.csv")
            ...
            trajectories = Parallel(n_jobs=16, verbose = 1)(delayed(add_timestamp)(json.loads(trajectory_str)) for i, trajectory_str in enumerate(data))
            ...
            trajectories = {}
            for line in lines:
                flag, trajectory_id, seq_id, class_id, t, x, y, speed, next_x, next_y = line.split('\t')
    ...
    def generate_experimental_and_correlation_dataset(dataset, copies=5):
        ...
        range_restricted_trajectories = list(filter(lambda x: TrajectoryUtil.filter_trajectory_in_range(x, Configuration.GPS_LIMIT), raw_trajectories))
        len(range_restricted_trajectories)
        ...
        for index in tqdm(range(copies)):
            ...
            selected_indexes = random.choice(range(len(exp_trajectories)), 1000, replace=False)

            rest_indexes = [i for i in range(len(exp_trajectories)) if i not in selected_indexes]
            rest_trajectories = [x for i, x in enumerate(exp_trajectories) if i in rest_indexes]
            for trajectory in rest_trajectories:
                correlation_trajectories.append(trajectory)

            exp_trajectories = [x for i, x in enumerate(exp_trajectories) if i in selected_indexes]

            with open(Path(out_path, "correlation_trajectories_{}.dat".format(index)), "w") as f:
                json.dump(correlation_trajectories, f)

            exp_trajectories = list(map(lambda x: TrajectoryUtil.point_to_cell(x), exp_trajectories))
            with open(Path(Configuration.CLEANSED_DATA_PATH.format(dataset.value), "exp_trajectories_{}.dat".format(index)), "w") as f:

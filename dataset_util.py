        ...
        if dataset == Dataset.GEO_LIFE:
            ...
            def extract_geo_life_trajectory(file_name, out_file_name, out_file_path):
                with open(file_name, "r") as f:
                    trajectory_data = list(map(lambda line: (float(line[0]), float(line[1]), 24 * 3600 * float(line[4])),
            ...
            trajectories = Parallel(n_jobs=16, verbose=1)(
                delayed(extract_geo_life_trajectory)(file, file.name, Configuration.EXTRACTED_DATA_PATH.format(dataset.value)) for file in file_names)
            ...
            trajectories = Parallel(n_jobs=16, verbose=1)(
                delayed(add_timestamp)(json.loads(trajectory_str)) for i, trajectory_str in enumerate(data))

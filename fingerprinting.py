...
class Fingerprinting:
    ...
    def probabilistic_fingerprint(trajectory, tau, p, theta, correlation, debug=False):
        ...
        if p == 0:
            if debug: print("p = 0, return origin")
        ...
        if debug: print("First cell truth: %5d, %5d, %10.2f" % (x_cell, y_cell, true_time))

        ## Extract the emission
        distribution = correlation.get_emission((x_cell, y_cell))

        ## Sample from the emission probabilities
        (sampled_lat, sampled_lng), fp_state = Sampling.sample_proportionally_with_truth(distribution, (x_cell, y_cell),
                                                                                         p_current)
        if debug: print("Sampled: ", sampled_lat, sampled_lng, "FP:", fp_state)
        ...
        for true_lat, true_lng, true_time in trajectory[1:]:
            if debug: print("Prev:", prev_lat, prev_lng, "Truth: ", true_lat, true_lng)

            (sampled_lat, sampled_lng), fp_state = Sampling.sample_candidates((prev_lat, prev_lng), (true_lat, true_lng),
                                                                              p_current, tau, correlation, \
                                                                              replace= True, debug=False)

            if debug: print("Sampled: ", sampled_lat, sampled_lng, "FP:" ,fp_state, "(p =", p_current, ")")
            ...
            if block_count >= math.ceil(1 / p):
                ...
                if debug: print("FP_COUNT: %d, Expected: %.2f" % (fp_count, p * length))
